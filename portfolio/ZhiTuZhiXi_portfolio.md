...
- 项目一句话：将岗位-技能-课程-项目素材关系封装为统一 analyze_career 接口，用扩展演示数据保证公网可用，并预留 Neo4j、XGBoost 和 NER 模型接入。
...
- 产品目标：输入目标岗位和当前技能，即可获得技能差距、薪资区间、课程补齐路径、可横向投递岗位、推荐项目和面试追问。
...
| 推荐 | 按缺口技能匹配课程，同时根据岗位技能重叠度返回 similar_jobs，并从项目素材库匹配可写进简历的项目。 |
| 展示 | 用雷达图、薪资柱状图、课程卡片、项目卡片、相似岗位卡片和面试问题组织结果。 |

## 3. 核心界面展示

结果页同时给出模式标识、岗位画像、缺口技能数量、薪资中位数、相似岗位数量、推荐项目和技能差距表，适合在面试中现场演示产品闭环。
...
| recommended_projects | 从个人项目库中选择最适合该岗位的项目，返回匹配理由、项目摘要、技能标签和简历表述。 |
...
- demo_assets/career_graph_demo.json 存放岗位-技能-课程-项目样例数据，保证公网部署无需重型模型文件。
- DemoCareerAdapter 负责岗位匹配、技能差距计算、课程推荐、项目推荐、相似岗位排序和面试问题生成。
...
| 层级 | 实现 |
| --- | --- |
| 前端 | Streamlit 页面，左侧输入，右侧结果，Plotly 雷达图、薪资柱状图、项目卡片和面试准备页签。 |
...
- 以“数据分析师 + Python/Excel”为例，系统识别 6 个缺口技能，给出 13,000 元/月薪资中位数、8 个相似岗位和 5 个推荐项目。
- 扩展演示数据包含 22 个岗位样例、61 个技能标签和 11 个个人项目素材。
- 课程推荐聚焦 SQL、统计分析、Tableau、A/B 测试、业务理解和数据可视化等可补齐技能。
- 项目推荐会把 GrowthLens、低碳转型、信用风险、消费者洞察、行业财务因子等项目按岗位要求排序。

## 8. 产品亮点

- 把原本偏研究/分析的职位图谱项目，落成可在线访问的职业规划与简历项目推荐工具。
- 通过 adapter 设计把前端产品体验与后端图谱/模型解耦。
- 明确展示运行模式，让 demo 可公开演示且不夸大真实后端能力。
- 把个人项目库接入推荐逻辑，能直接回答“这个岗位我该主打哪个项目”。
